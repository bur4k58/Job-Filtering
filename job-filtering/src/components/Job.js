...
export default function Job({ job }) {
  ...
  return (
    <div className={`flex flex-col shadow-md mx-5 m-5 p-5 my-16 bg-white ${job.featured && 'border-l-4 border-primary rounded'} mobile:flex-row mobile:mx-20`}>
      <div>
        <img src={`${job.logo}`} alt={job.company} className="-mt-16 mb-4 w-16 h-16 mobile:mt-0 mobile:w-24 mobile:h-24"/>
      </div>
      <div className="flex flex-col justify-between px-2 mobile:ml-5">
      ...
      <div className="flex items-center flex-wrap border-t-2 pt-4 mt-4 mobile:ml-auto mobile:border-0 mobile:pt-0 mobile:mt-0">
        {languages.length > 0
