...
export default function Job({ job }) {
  ...
  return (
    <div className={`flex shadow-lg mx-20 m-5 mx-5 p-5 bg-white ${job.featured && 'border-l-4 border-primary'}`}>
      <div>
      ...
      <div className="flex flex-col ml-5 justify-between">
        <div className="flex space-x-3">
          ...
          {job.featured ? <span className="rounded-full px-2 p-1 bg-verydark text-white font-bold">FEAUTRED</span> : null}
        </div>
      ...
      <div className="flex items-center justify-center ml-auto">
        {languages.length > 0
          ? languages.map((x) => <p className="text-primary bg-background font-bold m-2 p-2 rounded" key={x}>{x}</p>)
          : null
        }
      </div>
    </div>
